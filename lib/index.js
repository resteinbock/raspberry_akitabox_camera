...
module.exports = _camera = {
    configure:function(app){
        ...
        _camera.time_lapse_interval_min = app.config.camera.photo_interval_min;
        ...
        async.series(
            [
                function(series_cb){
                    ...
                    if (_camera.app.config.camera.rpi_camera) {
                ...
                function(series_cb){
                    //send message to akitabot
                    var message = 'successfully took rpi pic =)         ' + _camera.app.config.domain + '/showpic/' + _camera.last_commit.revisions[0].document;
                    return _camera.app.ping_akitabot.pingDawgPound(message, series_cb);
                }*/
            ],
            callback
        );
            ...
            function(revision, each_cb){
                _camera.app.pv_client.post(
                        path.join(_camera.app.config.project.uri, '/docs', revision.document, '/tags'),
                    {
                        ...
                        value_alias: _camera.app.config.tag.value
                    },
                    function (err, res) {
                        if (err) return each_cb(err);
