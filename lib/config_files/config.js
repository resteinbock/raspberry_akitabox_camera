var path = require('path');

module.exports = {
    app_name: "rpi_camera",
    config_name: "config.js",
    local_fs_path: path.join(__dirname, '/../../tmp/'),

    log_path: path.join(__dirname, '/../../logs/camera.log'),
    num_log_lines: 100,

    port: 8080,
    domain: "http://akitabox.ddns.net:" + this.port,

    client_id: "rpi_robbie",
    client_secret: "42ZWG91uL0aKPlLNdqW1oruQ0flT4a8A",

    api_server: "api.akitabox.com",
    api_protocol: "https",
    debug: false,

    camera: {
        secret: "h0114",

        rpi_camera: true,

        local_zips: path.join(this.local_fs_path, '/zips'),
        local_photos: path.join(this.local_fs_path, '/photos'),

        min_photo_interval: 2*60*1000, //2 min

        //photo_interval_min: 4,
        photo_interval_min: 10,
        //photo_interval_min: 60,

        //mode: "motion"
        mode: "time_lapse"
        //mode: "live_feed"
        //mode: "on_demand_only"
    },

    days: [false, true, true, true, true, true, false],
    hours: {
        start: 7,
        stop: 19
    },

    project:{
        //uri:"/shayden_turissystems/primex_thermometer",
        uri:"/rsteinbock_turissystems/rpi",
        access_token: ""
    },

    tag: {
        name: "camera",
        value: "conference_room"
    },

    doci_url: "https://doci.akitabox.com",

    akitabot_domain: "akitabot.akitabox.com",
    akitabot_secret: "h0114"
};
